...
export const History: React.FC<HistoryProps> = ({ history, onBack, onStartNew }) => {
  // Simple Mock Stats based on history
  const totalTrophies = history.filter(h => h.status === 'winner').length;
  const totalMatches = history.reduce((acc, h) => acc + h.matches.length, 0);
  const totalWins = history.reduce((acc, h) => acc + h.matches.filter(m => m.result === 'WIN').length, 0);
                                ...
                                <div key={m.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-background-dark/50 border border-slate-100 dark:border-primary/5">
                                    ...
                                    <div className="flex-1 text-right flex justify-end">
                                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                                            m.result === 'WIN' ? 'bg-primary/20 text-primary' :
